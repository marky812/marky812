...
for week_start in range(0, 30, 7):
    ...
    for i, day in enumerate(calendar_days[week_start:week_start + 7]):
        ...
        if has_entry:
            calories_for_day = int(df_by_date[day_str].get("calories", 0))
            minutes_for_day = int(df_by_date[day_str].get("exercise_minutes", 0))
            label += f"\n🔥 {calories_for_day} cal\n🏃 {minutes_for_day} min"
        else:
            label += "\n—"
