    ...
    with mid:
        st.markdown(
            ...
            <div style="text-align:center;margin-top:7vh">
              ...
              <div style="font-family:'Fraunces',Georgia,serif;font-size:1.55rem;
                          font-weight:600;color:var(--ink);margin-top:8px">Daily Wellness</div>
              <div style="color:var(--muted);font-size:.9rem;margin-top:3px">
...
st.markdown(
    ...
    :root{
      --tw-1:#F97316; --tw-2:#EA580C; --tw-3:#C2410C;
      --cal-soft:#FEF3C7; --cal-ink:#B45309;
      --pro-soft:#CCFBF1; --pro-ink:#0F766E;
      --ink:#1F1A14; --muted:#766E64; --line:#EDE7E0; --card:#FFFFFF; --bg-soft:#FFFBF6;
      --dot:#EA580C;
    }
    ...
    div[data-testid="stButton"] > button:hover{ border-color:#e3c4a6; transform:translateY(-1px); }
    button[kind="primary"], button[kind="primaryFormSubmit"]{
      background-image:linear-gradient(135deg,var(--tw-1),var(--tw-2)) !important;
      border:0 !important; color:#fff !important;
      box-shadow:0 8px 18px -10px rgba(234,88,12,.6) !important;
    }
    ...
    .hero{
      ...
      background:linear-gradient(125deg,#F97316 0%,#EA580C 52%,#C2410C 100%);
      color:#fff; padding:26px 30px; border-radius:24px;
      box-shadow:0 16px 40px -16px rgba(234,88,12,.55); margin-bottom:20px;
    }
    ...
    .today-pill{ font-size:.8rem; font-weight:700; color:var(--tw-2); text-decoration:none; background:#FFEDD5; padding:6px 13px; border-radius:999px; }
    ...
    .day:hover{ transform:translateY(-2px); box-shadow:0 10px 22px -12px rgba(46,30,20,.26); border-color:#ead7c2; }
    ...
    .mon-tag{ font-size:.58rem; font-weight:800; color:#fff; background:#B8A99B; padding:1px 5px; border-radius:6px; margin-left:5px; vertical-align:middle; }
    .day-empty{ color:#e4d6c6; font-weight:700; font-size:1.05rem; margin:auto; }
    ...
    .dot{ width:8px; height:8px; border-radius:50%; background:transparent; border:1.5px solid #e0d2c2; }
    .dot.on{ background:var(--dot); border-color:var(--dot); }

    .day.is-logged{ border-color:#FED7AA; background:linear-gradient(180deg,#fff,#fffaf4); }
    .day.is-today{ border-color:var(--tw-2); box-shadow:0 0 0 2px rgba(234,88,12,.2); }
    .day.is-selected{
      background:linear-gradient(140deg,var(--tw-1),var(--tw-2)); border-color:transparent; color:#fff;
      box-shadow:0 12px 24px -12px rgba(234,88,12,.62);
    }
    ...
    .day.is-future .day-empty{ color:#efe3d4; }
...
st.markdown(
    f"""
    <div class="sec-row">
      <div class="eyebrow" style="margin:0">Previous 7 days · this month · next month</div>
...
with totals_col:
    ...
    st.markdown(
        f'<div class="tcard"><div class="tnum">{habit_yes_count}/{len(BOOL_COLUMNS)}</div>'
...
def trend_bars(data, ycol, ylabel, colors):
    ...
    return (
        ...
        .configure_axis(
            labelColor="#766E64", titleColor="#766E64",
            domainColor="#EDE7E0", tickColor="#EDE7E0", gridColor="#F4EDE4",
        )
    )
